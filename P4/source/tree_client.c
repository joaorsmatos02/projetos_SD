...
int main(int argc, char* argv[]) {
    ...
    while(1) {
        ...
        //process the input
        inputHandler(inputTokens, tokenQuantity, argv[1]);
...
void inputHandler(char** inputTokens, int tokenQuantity, char* address_port) {
    ...
    if(strcmp(inputTokens[0], "put") == 0) {
        ...
        if(tokenQuantity == 3) {
            ...
            int verify = rtree_put(entry);
            result = 0;
            entry_destroy(entry);
            printf("O seu pedido é o número: %d\n", verify);
        }
    }
    else if(strcmp(inputTokens[0], "get") == 0) {
        if(tokenQuantity == 2) {

            struct data_t* data = rtree_get(inputTokens[1]);
        ...
        if(tokenQuantity == 2) {
            ...
            int verify = rtree_del(inputTokens[1]);
            result = 0;
            printf("O seu pedido é o número: %d\n", verify);
        }
    }
        ...
        if(tokenQuantity == 1) {
            ...
            char** values = (char**) rtree_get_values();
            result = 0;
    ...
    else if(strcmp(inputTokens[0], "verify") == 0) {
        if(tokenQuantity == 2) {

            int verify = rtree_verify(atoi(inputTokens[1]));
            result = 0;

            if(verify)
                printf("Operação já foi executada!\n");
            else
                printf("Operação ainda nao foi executada!\n");
        }
    }
...
void printMenu() {
    ...
    printf("verify <op_n>       - Verifica o estado da operação de escrita\n\n");
